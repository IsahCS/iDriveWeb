...
const Dashboard = () => {
  ...
  const handleEstimate = async () => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_URL_BACKEND}/ride/estimate`, {
        customer_id: userId,
  ...
  const handleConfirmRide = async (driver: DriverOption) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_URL_BACKEND}/ride/confirm`, confirmRideRequest);
